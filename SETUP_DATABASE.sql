-- ================================================
-- BETCHA COMPLETE DATABASE SCHEMA
-- Migration: Base tables + Social features + Group betting
-- Date: 2025-01-15
-- ================================================
...
CREATE TABLE IF NOT EXISTS profiles (
  ...
  CONSTRAINT valid_escrow_balance CHECK (escrow_balance >= 0)
);
...
CREATE TABLE IF NOT EXISTS bets (
  ...
  proof_url TEXT,
  challenge_type TEXT DEFAULT 'one_on_one' CHECK (challenge_type IN ('one_on_one', 'group_individual', 'team_vs_team', 'tournament')),
...
CREATE TABLE IF NOT EXISTS transactions (
  ...
  type TEXT NOT NULL CHECK (type IN ('deposit', 'withdrawal', 'bet_placed', 'bet_won', 'bet_lost', 'bet_refund', 'platform_fee', 'referral_bonus')),
...
-- ================================================
-- STEP 3: CREATE SOCIAL FEATURE TABLES
-- ================================================
...
-- ================================================
-- STEP 4: ADD CIRCULAR FOREIGN KEY CONSTRAINTS
-- ================================================

-- Add foreign keys that reference tables created after bets
ALTER TABLE bets
  ADD CONSTRAINT fk_bets_invited_via
  FOREIGN KEY (invited_via) REFERENCES challenge_invites(id) ON DELETE SET NULL;

ALTER TABLE bets
  ADD CONSTRAINT fk_bets_group_id
...
CREATE INDEX IF NOT EXISTS idx_bets_challenge_type ON bets(challenge_type);
...
CREATE INDEX IF NOT EXISTS idx_transactions_bet ON transactions(bet_id);
...
DROP POLICY IF EXISTS "Users can view bets they're involved in" ON bets;
CREATE POLICY "Users can view bets they're involved in" ON bets
  FOR SELECT USING (
    auth.uid() = creator_id OR
    auth.uid() = opponent_id OR
    EXISTS (
      SELECT 1 FROM challenge_participants
      WHERE challenge_participants.bet_id = bets.id
...
DROP POLICY IF EXISTS "Users can delete own pending bets" ON bets;
CREATE POLICY "Users can delete own pending bets" ON bets
  FOR DELETE USING (
...
-- Create default platform wallet
INSERT INTO platform_wallet (balance, total_fees_collected, total_payouts)
VALUES (0, 0, 0)
...
-- Function 1: Complete Group Individual Challenge
...
-- Function 2: Complete Team Challenge (Two Teams)
...
-- Function 3: Complete Tournament Challenge (3+ Teams)
